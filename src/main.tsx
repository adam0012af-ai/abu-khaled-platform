...
function App(){
  ...
  useEffect(()=>{let alive=true;const enforceViewport=()=>{let meta=document.querySelector('meta[name="viewport"]') as HTMLMetaElement|null;if(!meta){meta=document.createElement("meta");meta.name="viewport";document.head.appendChild(meta)}meta.content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"};const cleanAuthUrl=()=>{const url=new URL(window.location.href);if(url.searchParams.has("code")||url.searchParams.has("error")||url.hash){window.history.replaceState({,original_price:"400 د.إ",deal_price:"340 د.إ",gallery:["https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=1200&q=92","https://images.unsplash.com/photo-1584917865442-de89df76afd3?auto=format&fit=crop&w=1200&q=92","https://images.unsplash.com/photo-1529139574466-a303027c1d8b?auto=format&fit=crop&w=1200&q=92"],highlights:["خصم إضافي على تشكيلات مختارة","الكود يطبق على المنتجات المؤهلة","إتمام الطلب من متجر نمشي الرسمي"]},document.title,window.location.pathname)}};const applyAuthenticatedSession=(session:any)=>{if(!alive||!session?.user)return;enforceViewport();setUser(session.user);setAuthOpen(false);setAuthBusy(false);setAuthMsg("");cleanAuthUrl()};supabase.auth.getSession().then(({data,error})=>{if(!alive)return;if(error){setAuthMsg(error.message);setAuthBusy(false);return}if(data.session)applyAuthenticatedSession(data.session);else setUser(null)});const{data:{subscription}}=supabase.auth.onAuthStateChange((_event,session)=>{if(!alive)return;if(session?.user)applyAuthenticatedSession(session);else setUser(null)});return()=>{alive=false;subscription.unsubscribe()}},[]);
